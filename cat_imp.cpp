...
void catalogue::output_id() {
    std::cout << catalogue_id;
}
...
std::ostream& operator<<(std::ostream& os, catalogue& c) {
    ...
    do {
        std::cout << "Please enter the type of astronomical object you wish to display" << std::endl;
        try {
            std::cin >> input;
    ...
    bool continue_read = false;
        while (!continue_read) {
            ...
            if (input == "planet") {
                ...
                do {
                    try {
                        std::cout << "Enter in the following order (comma-seperated): id, right-ascension[\370], declination[\370], redshift, mass[M_E], radius[R_E], orbital period[Days], planet type"<<std::endl;
                        std::cin.ignore();
                ...
                std::cout << " Do you want to add another object to this catalogue? (y/n)" << std::endl;
                std::cin >> user_choice;
                if (user_choice == "y") {
                    continue_read = false;
                }
                ...
                do{
                    try {
                        std::cout << "Enter in the following order (comma-seperated): id, right-ascension(\370), declination(\370), redshift, mass(M_E), radius(R_E), orbital period(days), planetary system" << std::endl;
                        std::cin.ignore();
                        getline(std::cin, buffer);
                ...
                do {
                    try {
                        std::cout << "Enter in the following order (comma-seperated): id, right-ascension[\370], declination[\370], redshift, mass[log(M_sol)],radius[kpc],"<<std::endl;
                        std::cout<<"magnitude[AB], age[Gyr], luminosity[Log(L_sol)], morphology, peculiar velocity[km / s]" << std::endl;
                        std::cin.ignore();
                        getline(std::cin, buffer);
                ...
                do {
                    try {
                        std::cout << "Enter in the following order (comma-seperated): id, right-ascension[\370], declination[\370], redshift, mass[M_sol]," << std::endl;
                            std::cout<<"radius[R_sol], magnitude[AB], age[Gyr], luminosity[L_sol], temperature[T_6], magnetic field strength[B_sol]" << std::endl;
                        std::cin.ignore();
                        getline(std::cin, buffer);
...
void catalogue::set_id(std::string const id) {
    catalogue_id = id;
}
...
void catalogue::sort(std::string type){
    ...
    if (type == "planet" || type == "moon") {
        ...
        // sort between range of temp_set. sorting function -- lambda that compares members of obj1,obj2 by calling general getter function
            ...
            std::sort(std::execution::par_unseq, temp_set.begin(), temp_set.end(),
                [choice_int](const std::unique_ptr<astronomical_object>& obj1,
                    const std::unique_ptr<astronomical_object>& obj2) {
                        return obj1->get_all_new()[choice_int] < obj2->get_all_new()[choice_int];
                });
        }
        else if (order == "2") {
            std::sort(std::execution::par_unseq, temp_set.begin(), temp_set.end(),
        ...
        do {
            ...
            try {
                non_satellite_sort_choice = regex_functions::between_1_11(choice_nonsatellite);
            }
            catch (std::runtime_error& e) {
                std::cout << e.what() << std::endl;
            }
        ...
        if (order_choice == "1") {
            std::sort(std::execution::par_unseq, temp_set.begin(), temp_set.end(), [choice_int](const std::unique_ptr<astronomical_object>& obj1,
                const std::unique_ptr<astronomical_object>& obj2) {
                    return obj1->get_all_new()[choice_int] < obj2->get_all_new()[choice_int];
                });
        }
    ...
    objects.insert(objects.end(), std::make_move_iterator(temp_set.begin()), std::make_move_iterator(temp_set.end()));
